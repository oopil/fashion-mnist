...
def CNN_model():
    ...
    model.add(Conv2D(filters=156, kernel_size=(3, 3), padding='Same',
                     activation='relu'))
    model.add(Conv2D(filters=156, kernel_size=(3, 3), padding='Same',
                     activation='relu'))
    model.add(BatchNormalization())
...
if __name__ == "__main__":
    ...
    test_depend_encode = encode_dependancy(test_one_hot)
    # ------- Model training----- #
    model = CNN_model()
    adam = Adam(lr=LR, beta_1=0.9, beta_2=0.999, epsilon=None, decay=LR*(1/epoch), amsgrad=False)
    model.compile(loss='mean_squared_error', optimizer=adam, metrics=['accuracy'])
    history = model.fit(x=x_train_norm, y=t_depend_encode, epochs=epoch, batch_size=6000, verbose=1, validation_data=(x_test_norm, test_depend_encode))
